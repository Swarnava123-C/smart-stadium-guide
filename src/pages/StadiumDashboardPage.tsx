...
export const StadiumDashboardPage: React.FC = () => {
  ...
  const { stadium, events, logs, loading } = useStadiumDetail(id);
  const { state } = useStadium();

  // Run simulation for events of this stadium
  ...
  const occupancyPct = liveEvent
    ? Math.round((liveEvent.current_attendance / stadium.capacity) * 100)
    : 0;

  // Surge detection from latest log
  ...
  const affectedGates = Object.entries(gateStatuses)
    .filter(([, s]) => s === 'open')
    .map(([k]) => k.replace('_', ' ').replace('gate', 'Gate'));
    ...
    <div className="space-y-4">
      ...
      {state.isEmergencyMode && (
        <div className="glass rounded-xl p-4 border-destructive/50 bg-destructive/10 flex items-center gap-3 animate-pulse" role="alert">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">All gates forced open. AI recommendations paused. Evacuation protocol engaged.</p>
          </div>
        </div>
      )}
      ...
      {liveEvent && !state.isEmergencyMode && (
        <GateRecommendation
          ...
          expectedAttendance={liveEvent.expected_attendance}
        />
      )}
      ...
      {liveEvent && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <StatCard
              title="Attendance"
              value={`${(liveEvent.current_attendance / 1000).toFixed(1)}K`}
            ...
            <StatCard
              ...
              subtitle={avgWaitTime > 8 ? 'Above threshold' : 'Normal'}
              icon={<Clock className="w-4 h-4" />}
              variant={avgWaitTime > 8 ? 'danger' : 'default'}
            />
            <StatCard
              title="Gates Open"
              value={`${Object.values(gateStatuses).filter(s => s === 'open').length}/${Object.keys(gateStatuses).length || 4}`}
                ...
                <div key={event.id} className="px-4 py-3 flex items-center justify-between cursor-pointer hover:bg-muted/30 transition-colors" onClick={() => navigate(`/event/${event.id}`)}>
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {new Date(event.event_date).toLocaleDateString()} • Expected: {(event.expected_attendance / 1000).toFixed(0)}K
                    </p>
                  </div>
                  <div className="text-right">
                    <p className={cn('text-xs font-semibold', riskColor)}>
                      🔥 {riskLevel} Risk ({riskPct}%)
                    </p>
      ...
      {completedEvents.length > 0 && (
        <div className="glass rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-border/30 flex items-center gap-2">
            ...
            <h3 className="font-display font-semibold text-sm">Past Events</h3>
          </div>
              ...
              <tbody>
                {completedEvents.map(event => (
                  <tr key={event.id} className="border-b border-border/10 cursor-pointer hover:bg-muted/30 transition-colors" onClick={() => navigate(`/event/${event.id}`)}>
                    ...
                    <td className="px-4 py-2.5 font-mono">{Math.round((event.current_attendance / stadium.capacity) * 100)}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
